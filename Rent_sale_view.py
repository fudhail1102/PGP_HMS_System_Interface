...
from _1BHKStructure import ProvidentGreenPark_Rent_1
from _2BHKStructure import ProvidentGreenPark_Rent_2
from _3BHKStructure import ProvidentGreenPark_Rent_3
from SaleStructure import ProvidentGreenPark_Sale
from path_relative import *

def OutsiderView(option):
    window = tk.Tk()
    window.title("Apartment Details")
    window.geometry("1200x613")
    window.configure(background="#507B3B")
    if option==1:
        apartment = ProvidentGreenPark_Rent_1
    elif option==2:
        apartment= ProvidentGreenPark_Rent_2
    elif option==3:
        apartment =ProvidentGreenPark_Rent_3
    else:
        apartment=ProvidentGreenPark_Sale
    pos = None
    HouseWindow = None
    ...
    def open_house_view(house):
        ...
        column = 10
        for key, value in house.data.get_user_details().items():
            if key!="status":
